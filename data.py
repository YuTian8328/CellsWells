...
def get_feature_names(features, channel_markers=CHANNEL_MARKERS):
    ...
    for feat in features:
        if feat in ["distance", "edge_type"]:
            ...
            feat_names.extend(["coord-x", "coord-y", "coord-z"])
        elif feat == "size":
            feat_names.extend(["area", "volume"])
        elif feat == "marker":
...
def nx_to_tg_graph(G,
    ...
    for node_ind in G.nodes:
        ...
        data["y"].append(feat_val[2:7])
    

    for edge_ind in G.edges:
        feat_val = []
        for key in edge_features:
            feat_val.extend(process_feature(G, key, edge_ind=edge_ind, **kwargs))
...
class GNNDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 transform=[],
                 pre_transform=None,
                 graph_folder_name="graphs_from_wells",
        ...
        self.node_feature_names = get_feature_names(node_features, channel_markers=self.channel_markers)
        self.edge_feature_names = get_feature_names(edge_features, channel_markers=self.channel_markers)
